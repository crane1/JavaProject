...
public class UserRequestFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		if(stu == null){
			...
			if( (path.contains("/teamLeader/") && !user_right.equals("t_leader"))
				|| (path.contains("/classMonit/") && !user_right.equals("c_monitor"))
				|| (path.contains("/classManger/") && !user_right.equals("c_man_teacher"))
				|| (path.contains("/teacher/") && !user_right.equals("c_teac_teacher"))
				|| (path.contains("/admin/") && !user_right.equals("normal_manager"))
				|| (path.contains("/superAdmin/") && !user_right.equals("admin"))){
				
				res.sendRedirect(root + "/fail.jsp");
				return;
			}
		}
