...
public class TeamDAOImp1 implements ITeamDAO {
	...
	public Team findTeamById(String id) {
		String sql = "SELECT s.t_id,s.t_name,s.t_leader,s.class_id FROM sims_team s WHERE s.t_id='"
				+ id + "'";
		return jt.query(sql, new ResultSetExtractor<Team>() {
			...
			public Team extractData(ResultSet rs) throws SQLException,
					DataAccessException {
				if (rs.next()) {
					return new Team(rs.getString("t_id"), rs
							.getString("t_name"), rs.getString("t_leader"), rs
							.getString("class_id"));
				}
				return null;
			}
	...
	public int modifyTeam(Team team) {
		String sql = "update SIMS_TEAM t " + "set t.t_name = '"
				+ team.getName() + "', t.t_leader = '" + team.getLeaderID()
				+ "', t.class_id= '" + team.getClassID() + "' where t.t_id='"
				+ team.getId() + "'";
		return jt.update(sql);
	}
	...
	// String
	// sql="SELECT s.t_id,s.t_name,s.t_leader,s.class_id FROM sims_team s WHERE s.class_id='"+classID+"'";
	...
	private Team createTeam(ResultSet rs) {
		Team ta = null;
		try {
			ta = new Team(rs.getString("t_id"), rs.getString("t_name"),
					rs.getString("t_leader"), rs.getString("class_id"));
	...
	public List<Student> findStudentByTeam(String classid) {
		String sql = null;
		sql = "select s.s_id,s.s_name,s.s_sex,s.s_age,s.s_gradfrom,s.s_tel,s.s_addr,s.s_idcard,s.s_email,s.s_teamid from sims_student s where s.s_team is null and s.s_classid='"
				+ classid + "'";
		return jt.query(sql, new RowMapper<Student>() {
			...
			public Student mapRow(ResultSet rs, int index) throws SQLException {
				if (rs.next()) {
					...
					rs.getString("s_id"), rs.getString("s_name"), rs
							.getString("s_sex"), rs.getInt("s_age"), rs
							.getString("s_gradFrom"), rs.getLong("s_tel"), rs
							.getString("s_addr"), rs.getString("s_idcard"), rs
							.getString("s_email"), rs.getString("s_teamid"), rs
							.getString("s_classid"));
				}
				return null;
			}
	...
	public int updateTeam(String teamId, String id) {
		String sql = null;
		sql = "update sims_student  set s_teamid='" + teamId + "' where s_id='"
				+ id + "'";
		return jt.update(sql);
	}
	...
	private List<Team> getQueryPage(String attrStr, String conditionStr,
			String conditionId) {
		List<Team> list = null;
		String sql = "{?=call getSimPageRows(?,?,?,?,?,?,?)}";
		list = (List<Team>) jt.execute(new CallableStatementCreator() {
			...
			@Override
			public List<Team> doInCallableStatement(CallableStatement ts)
					throws SQLException, DataAccessException {
				List<Team> resultsList = new ArrayList<Team>();
				ts.execute();
				PageManager.setMaxRowNumber((Integer) ts.getObject(7)); // 设置从数据库中拿到的所有记录条数，并设置到PageManager里
