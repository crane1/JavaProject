...
public class ClassDAOImp implements IClassDAO {
	...
	public CClass findClassById(String id) {
		// 构建sql
		String sql = "select " + CONDITION + " from SIMS_CLASS  where c_id='"
				+ id + "'";
		return jt.query(sql, new ResultSetExtractor<CClass>() {
			...
			public CClass extractData(ResultSet rs) throws SQLException,
				...
				if (rs.next()) {
					return new CClass(rs.getString("c_id"), rs
							.getString("c_name"), rs.getString("c_monitor"), rs
							.getString("c_man_teacher"), rs
							.getString("c_teac_teacher"));
				}
				return null;
			}
		});
	...
	public int addClass(CClass cclass) {
		String sql = "insert into SIMS_CLASS values('" + cclass.getId() + "','"
				+ cclass.getName() + "','"
				+ (cclass.getMonitorID() == null ? "" : cclass.getMonitorID())
				+ "','" + cclass.getManTeacherID() + "','"
				+ cclass.getTeacTeacherID() + "')";
	...
	public int modifyClass(CClass cclass) {
		String sql = "update SIMS_CLASS s "
				...
				+ (cclass.getMonitorID() == null ? "" : cclass.getMonitorID())
				+ "', s.c_man_teacher='"
				+ (cclass.getManTeacherID() == null ? "" : cclass
						.getManTeacherID())
				+ "', s.c_teac_teacher='"
				+ (cclass.getTeacTeacherID() == null ? "" : cclass
						.getTeacTeacherID()) + "' where s.c_id='"
				+ cclass.getId() + "'";
		return jt.update(sql);
	}
	...
	private CClass createClass(ResultSet rs) {
		...
		try {
			cc = new CClass(rs.getString("c_id"), rs.getString("c_name"),
					rs.getString("c_monitor"), rs.getString("c_man_teacher"),
	...
	// 进行分页查询
	private List<CClass> getQueryPage(String attrStr, String conditionStr,
			String conditionId) {
		List<CClass> list = null;

		String sql = "{?= call getSimsPageRows(?,?,?,?,?,?,?)}";

		list = (List<CClass>) jt.execute(new CallableStatementCreator() {
			...
			public List<CClass> doInCallableStatement(CallableStatement cs)
					throws SQLException, DataAccessException {
				List<CClass> resultsList = new ArrayList<CClass>();
				cs.execute();
				PageManager.setMaxRowNumber((Integer) cs.getObject(7)); // 设置从数据库中拿到的所有记录条数，并设置到PageManager里
