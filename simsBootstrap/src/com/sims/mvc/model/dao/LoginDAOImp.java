...
public class LoginDAOImp implements ILoginDAO {
	...
	public Student checkLogin(String id, String pwd) {
		...
		String sql = "select s.s_id,s.s_pwd,s.s_name,s.s_sex,s.s_age,s.s_gradfrom,s.s_tel,s.s_addr,s.s_idcard,s.s_email,s.s_team,s.s_class,s.t_leader,s.c_monitor,s.c_man_teacher,s.c_teac_teacher,s.normal_manager from sims_student s where s.s_id='"
				+ id + "'";
		Student stu = null;
	...
	public static String checkPermi(String per, String id) {
		System.out.println("checkPermi(String per, String id)===" + per);
		String rec = "";
		if (per.equals("stu")) {
			rec = "success";
		} else {
